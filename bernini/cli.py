...
"""Shared command-line plumbing for full Bernini inference scripts."""
...
from .pipeline import BerniniPipeline
...
def add_common_args(parser):
    ...
    g.add_argument(
        "--config",
        default="configs/bernini",
        help="full Bernini Diffusers directory or config directory",
    )
    g.add_argument("--high_noise_ckpt", default=None, help=argparse.SUPPRESS)
    g.add_argument("--low_noise_ckpt", default=None, help=argparse.SUPPRESS)
...
def build_pipeline(args, device):
    ...
    raise ValueError(
        ...
        "ComfyUI-RH-Bernini-Full supports only the full "
        "Bernini-Diffusers model_type='bernini' pipeline and does not load Bernini-R."
    )
