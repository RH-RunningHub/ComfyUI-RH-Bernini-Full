...
try:
    ...
    def gather_heads_scatter_seq(tensor, *args, **kwargs):
        return tensor

    def gather_seq_scatter_heads(tensor, *args, **kwargs):
        return tensor

    def pad_tensor(tensor, dim, padding_size, *args, **kwargs):
        if padding_size <= 0:
            return tensor
        pad = [0, 0] * tensor.dim()
        pad_index = 2 * (tensor.dim() - dim - 1) + 1
        pad[pad_index] = padding_size
        return F.pad(tensor, tuple(pad))

    def reduce_sequence_parallel_loss(loss, *args, **kwargs):
        return loss

    def unpad_tensor(tensor, dim, padding_size, *args, **kwargs):
        if padding_size <= 0:
            return tensor
        slices = [slice(None)] * tensor.dim()
        slices[dim] = slice(0, tensor.shape[dim] - padding_size)
