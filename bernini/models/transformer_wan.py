...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def prepare_inputs_for_sp(
        ...
        timestep_proj_indices: Optional[torch.Tensor],
        temb: torch.Tensor,
    ):
        """Shard inputs across the Ulysses group and build cu_seqlens metadata.

        With Ulysses disabled, the slicing/padding steps are skipped and the
        single packed sample can use broadcast timestep embeddings instead of
        materializing one copy per token.
        """
    ...
    def forward(
        ...
        if use_per_token_timestep:
            # Expand only when sequence parallelism or multiple packed samples
            # require different timestep embeddings per token.
